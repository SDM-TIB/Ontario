...
from . import Query, RDFTerm, Triple, UnionBlock, JoinBlock, Optional, Filter, Expression
...
def p_express_arg_0(p):
    ...
    express_arg : uri
    """
    p[0] = RDFTerm(p[1], True, isuri=True)
...
def p_express_arg_2(p):
    ...
    """
    c = p[1].strip()
    p[0] = RDFTerm(p[1], True)
    if xsd in p[1]:
        p[0] = RDFTerm(c[:c.find("^")], True, datatype=c[c.rfind("^")+1:])
    if "@" in p[1]:
        p[0] = RDFTerm(c[:c.find("^")], True, datatype="<" + xsd + "string>", lang=c[c.rfind("@")+1:])
...
def p_express_arg_03(p):
    ...
    p[0] = RDFTerm(decimalNumber, True)
...
def p_express_arg_4(p):
    ...
    """
    p[0] = Expression("REGEX", p[3], RDFTerm(p[5], False, p[6]))
...
def p_predicate_rdftype(p):
    ...
    if p[1] == 'a':
        ...
        p[0] = RDFTerm(value, True, isuri=True)
    else:
